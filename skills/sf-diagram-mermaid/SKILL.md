...
metadata:
  ...
  short-description: Creates Salesforce architecture diagrams using Mermaid with ASCII fallback. Use when visualizing OAuth flows, data models (ERDs), integration sequences, system landscapes, role hierarchies, or Agentforce agent architectures.
---
...
export SKILL_DIR="$CODEX_HOME/skills/sf-diagram-mermaid"
...
1. **Marketplace folder** (always available): `$CODEX_HOME/skills/sf-diagram-mermaid/templates/[template]`
2. **Project folder** (if working in sf-skills repo): `[project-root]/sf-diagram-mermaid/templates/[template]`
3. **Cache folder** (if installed individually): `$CODEX_HOME/skills/sf-diagram-mermaid/templates/[template]`

**Example**: To load JWT Bearer template:
```
Read: $CODEX_HOME/skills/sf-diagram-mermaid/templates/oauth/jwt-bearer.md
```
...
```bash
python3 $CODEX_HOME/skills/sf-diagram-mermaid/scripts/query-org-metadata.py \
